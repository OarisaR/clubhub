...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _signup() async {
    ...
    if (_selectedInterests.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select at least one field of interest'), backgroundColor: Colors.orange),
      );
      return;
    }
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Select Your Interests', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.primary)),
                    TextButton(
                      onPressed: () {
                        setState(() {});
                        Navigator.pop(context);
                ...
                child: Padding(
                  ...
                  child: GridView.builder(
                    gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                        crossAxisCount: 3, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 2.5),
                    itemCount: _availableInterests.length,
                    itemBuilder: (context, index) {
                      final interest = _availableInterests[index];
                      final isSelected = _selectedInterests.contains(interest);
                      return GestureDetector(
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                            color: isSelected ? AppColors.accent : AppColors.secondaryBackground.withOpacity(0.3),
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(
                              color: isSelected ? AppColors.accent : AppColors.secondaryBackground.withOpacity(0.5),
                              width: 1.5,
                            ),
                          ),
                          child: Center(
                ...
                child: Form(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildTextField(controller: _nameController, hintText: 'Full name', icon: Icons.person_outline, validator: (v) => v!.isEmpty ? 'Enter full name' : null),
                      const SizedBox(height: 16),
                      _buildTextField(controller: _emailController, hintText: 'Email address', icon: Icons.email_outlined, validator: (v){if(v==null||v.isEmpty)return 'Enter email'; if(!v.contains('@')) return 'Enter valid email'; return null;}),
                      const SizedBox(height: 16),
                      _buildTextField(controller: _passwordController, hintText: 'Password', icon: Icons.lock_outline, obscureText: _obscurePassword, validator: (v){if(v==null||v.isEmpty)return 'Enter password'; if(v.length<6)return 'Password must be at least 6 characters'; return null;}),
                      const SizedBox(height: 16),
                      GestureDetector(
                        onTap: _showInterestSelector,
                        child: Container(
  ...
  Widget _buildTextField({required TextEditingController controller, required String hintText, required IconData icon, bool obscureText = false, String? Function(String?)? validator}) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 8),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)),
      child: TextFormField(controller: controller, obscureText: obscureText, decoration: InputDecoration(hintText: hintText, prefixIcon: Icon(icon), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16)), validator: validator),
    );
  }
}
...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _signup() async {
    ...
    try {
      ...
      // Create Student object
      final student = Student(
        studentId: userCredential.user!.uid, // String UID
        name: _nameController.text.trim(),
        email: _emailController.text.trim(),
        password: _passwordController.text,
        year: _yearController.text.trim(),
      ...
      // Navigate to HomeScreen with Student object
      /*Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => HomeScreen(student: student),
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                            color: isSelected
                                ? AppColors.accent
                                : AppColors.secondaryBackground.withOpacity(
                                    0.3,
                                  ),
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                _buildTextField(
                  controller: _deptController,
                  hintText: 'Department (e.g., CSE, EEE)',
                  icon: Icons.school_outlined,
                  validator: (v) => v!.isEmpty ? 'Enter department' : null,
                ),
                const SizedBox(height: 16),
